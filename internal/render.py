...
def volumetric_rendering(rgbs,
  ...
  # render the illumination map
  # integral_by_weights = lambda values_along_rays: (jax.lax.stop_gradient(weights[..., None]) * values_along_rays).sum(axis=-2)
  integral_by_weights = lambda values_along_rays: (weights[..., None] * values_along_rays).sum(axis=-2)
  if 'L' in extras and extras['L'] is not None:
    # L.shape: [bs, num_samples]
    assert extras['R'] is not None
    rendering['L'] = integral_by_weights(extras['L'])
    if extras['coeff_gamma'] is not None:
      rendering['gamma'] = integral_by_weights(extras['coeff_gamma'])
    if extras['coeff_alpha'] is not None:
      rendering['alpha'] = integral_by_weights(extras['coeff_alpha'])
    rendering['R'] = integral_by_weights(extras['R'])
    if extras['coeff_beta'] is not None:
      rendering['beta'] = integral_by_weights(extras['coeff_beta'])

    # enhance L in test time (using two manners):
    # Step II: Add a new item to `rendering`
    rgb_padding = 0.001
    factor = 8
    L_enhances = {
      # 'L_avg0.6': extras['L'] * (0.6 / extras['L'].mean()),
      f'Lx{factor}': extras['L'] * factor,
      'Lg22': extras['L'] ** jnp.array([0.45, 0.45, 0.45]) * 1.5,
      f'L_avg_g22_x{factor}': (extras['L'] * factor + extras['L'] ** 0.45) / 2
    }
    for key, L_enhanced in L_enhances.items():
      # L_enhanced = L_enhance_func()
      rendering[key] = integral_by_weights(L_enhanced)
      rgb_along_rays = extras['R'] * L_enhanced * (1 + 2 * rgb_padding) - rgb_padding
      rendering[f'rgb_{key}'] = integral_by_weights(rgb_along_rays) + bg_w * bg_rgbs

  if 'rgb_enhanced' in extras and extras['rgb_enhanced'] is not None:
    rendering['rgb_enhanced'] = integral_by_weights(extras['rgb_enhanced']) + bg_w * bg_rgbs
    rendering['L_enhanced'] = integral_by_weights(extras['L_enhanced'])
