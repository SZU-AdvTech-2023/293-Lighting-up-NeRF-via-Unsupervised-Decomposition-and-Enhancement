...
def main(unused_argv):
  ...
  print_func(f'Number of parameters being optimized: {num_params}')
  ...
  for step, batch in zip(range(init_step, num_steps + 1), pdataset):
    ...
    if jax.host_id() == 0:
      ...
      if step == 1 or step % config.checkpoint_every == 0:
        ...
        if step == 1:
          print('Saving ckpt at step 1 only for testing bugs. Now remove checkpoint_1.')
          os.remove(os.path.join(config.checkpoint_dir, 'checkpoint_1'))

    # Test-set evaluation.
    if isinstance(config.valid_steps, Iterable):
      valid_condition = step in config.valid_steps
    else:
      assert type(config.valid_steps) == int
      valid_condition = (config.valid_steps > 0 and step % config.valid_steps == 0)
    ...
    if valid_condition:
      ...
      if jax.host_id() == 0:
        ...
        print_func(f'Eval {step}: {eval_time:0.3f}s., {rays_per_sec:0.0f} rays/sec')

        metric_start_time = time.time()
        metric = metric_harness(
          postprocess_fn(rendering['rgb']), postprocess_fn(test_case.rgb))
        print_func(f'Metrics computed in {(time.time() - metric_start_time):0.3f}s')
        for name, val in metric.items():
          if not np.isnan(val):
            print_func(f'{name} = {val:.4f}')
        ...
        print_func(f'Visualized in {(time.time() - vis_start_time):0.3f}s')
        if config.rawnerf_mode:
        ...
        for k, v in vis_suite.items():
          if v is not None:
            ...
            print(f'vis[{k}] is None, skip logging this item.')
